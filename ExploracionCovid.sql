...
SELECT location,date,total_cases,population,(total_cases/population)*100 as porcentaje_p
FROM covid_deaths;

-- pais con el mayor indice de contagios 
SELECT location,(total_cases/population)*100 as porcentaje
FROM covid_deaths
WHERE (total_cases/population)*100 = (SELECT(MAX((total_cases/population)*100)) FROM covid_deaths);
